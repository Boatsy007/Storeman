export default async function handler(req, res) {
  ...
  const photos = Array.isArray(body.photos)
    ? body.photos.slice(0, 3).map((photo) => ({
        name: clean(photo?.name).slice(0, 120) || 'site-photo.jpg',
        type: clean(photo?.type).slice(0, 80) || 'image/jpeg',
  ...
  const stageLabels = {
    ...
    quote_requested: 'Free quote requested — arrange site assessment'
  };
  ...
  if (resendKey) {
    ...
    const ownerHtml = `
      <div style="font-family:Arial,sans-serif;max-width:640px;margin:auto;color:#111">
        ...
        <div style="padding:26px;border:1px solid #e5e5e5;border-top:0">
          ...
          ${lead.notes ? `<p><strong>Anything else:</strong> ${lead.notes}</p>` : ''}
          ${lead.photoNames.length ? `<p><strong>Photos:</strong> ${lead.photoNames.length} attached</p>` : ''}
          <p><strong>Next action:</strong> ${lead.stage === 'quote_requested' || lead.siteVisitFollowUp ? 'Contact customer to arrange free site assessment and prepare quote' : 'Follow up enquiry'}</p>
    ...
    try {
      ...
      if (photos.length) {
        emailBody.attachments = photos.map((photo) => ({ filename: photo.name, content: photo.content }));
      }
      ...
      const customerHtml = `
        <div style="font-family:Arial,sans-serif;max-width:640px;margin:auto;color:#111">
          ...
          <div style="padding:28px;border:1px solid #e5e5e5;border-top:0">
            <p style="margin:0 0 8px;font-size:12px;font-weight:700;letter-spacing:1px">QUOTE REQUEST RECEIVED</p>
            ...
            <div style="background:#090909;color:#fff;padding:22px 24px;margin:22px 0;border-left:7px solid #ffe000">
              ...
              <div style="font-size:24px;font-weight:900;color:#ffe000;margin-top:8px">WE’LL CONTACT YOU TO ARRANGE A FREE SITE ASSESSMENT.</div>
              <p style="line-height:1.5;color:#eee;margin-bottom:0">We’ll confirm a suitable time, assess the scope of work and prepare your accurate quote.</p>
            </div>
            <p><strong>Business:</strong> ${lead.business}</p>
            <p><strong>Contact:</strong> ${lead.name}</p>
            <p><strong>Phone:</strong> ${lead.phone}</p>
            ...
            ${lead.notes ? `<p><strong>Anything else:</strong> ${lead.notes}</p>` : ''}
            <p style="margin-top:24px"><strong>Storeman</strong><br>1300 STOREMAN<br>1300 786 736<br>hello@storeman.com.au</p>
          </div>
        </div>`;
